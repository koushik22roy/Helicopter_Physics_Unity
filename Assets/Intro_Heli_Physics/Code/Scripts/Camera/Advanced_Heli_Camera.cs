...
public class Advanced_Heli_Camera : Base_Heli_Camera, IHeli_Camera
{
    #region Variables
    ...
    public void UpdateCamera()
    {
        ...
        RaycastHit hit;
        Ray groundRay = new(transform.position, Vector3.down);
        if (Physics.Raycast(groundRay, out hit, 100f))
        {
            if (hit.transform.CompareTag("ground") && hit.distance <= minGroundHeight)
            {
